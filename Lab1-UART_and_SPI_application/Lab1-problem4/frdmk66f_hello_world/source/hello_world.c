...
status_t SPI_read(uint8_t regAddress, uint8_t *rxBuff, uint8_t rxBuffSize)
{
	...
	masterTxData[0] = regAddress & 0x7F; //Clear the most significant bit, 7f is 01111111 therefore RW bits is 0 to perform a read operation
...
status_t SPI_write(uint8_t regAddress, uint8_t value){
	...
	masterTxData[0] = regAddress | 0x80; //set R/W bit to be 1 to perform write, and keep the remaining 7 bits
	masterTxData[1] = regAddress & 0x80; //picking the bit8 (ADDR[7])
...
int main(void)
{
    ...
    */
    SPI_read(0x15, &byte, 1);
    printf("The previous value is 0x%X\n", byte);
    status_t test = SPI_write(0x15,0x45);
    SPI_read(0x18, &byte, 1);
    printf("The expected value is 0x45 and the read value 0x%X\n", byte);
